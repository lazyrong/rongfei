...
<body>
	<div class="animated fadeInRight">
		...
		<div class="panel panel-default">
			<div class="panel-body">
				<strong>操作：</strong> <a class="btn btn-primary"
					href="javascript:void(0);" id="add">新增</a> <a
					class="btn btn-danger" href="javascript:void(0);" id="delAll">批量删除</a>
			</div>
		</div>
		...
		<div class="panel panel-default" style="margin-bottom: 0;">
			<table
				class="table table-bordered table-hover text-center vertical-middle-lg">
				<thead>
				...
				<tbody>
					<?php if(is_array($resultList)): foreach($resultList as $k=>$res): ?><tr>
						<td><input type="checkbox" name="ids[]"
							value="<?php echo ($res["news_id"]); ?>"><?php echo ($res["news_id"]); ?></td>
						<td><a href="<?php echo ($res["news_url"]); ?>"><?php echo ($res["name"]); ?></a><a
							href="<?php echo ($res["case_url"]); ?>"> [案例]</a></td>
						<td><?php echo ($res["cat_name"]["name"]); ?></td>
						<td><span class="text-danger"><?php echo ($res["price"]); ?></span></td>
						<td><?php echo (link2text($res["link_type"])); ?></td>
						<td><?php echo (sl2text($res["sl_type"])); ?></td>
						<td><?php echo ($res["remark"]); ?></td>
						<td><a href="<?php echo ($res["buy_link"]); ?>">下单</a></td>
						<td><?php echo ($res["pubtime"]); ?></td>
						<td><a
							href="<?php echo U('Admin/News/editNews',array('news_id'=>$res[news_id],'cat_id'=>$cat_id));?>"
							title="编辑"><span class="glyphicon glyphicon-edit"></span></a>&nbsp;&nbsp;<a
							href="<?php echo U('Admin/News/del',array('news_id'=>$res[news_id]));?>"
    	...
    	//询问框
    	  var index = layer.confirm('删除全部所选内容？', {
    	    ...
    	    //获取checkedbox value
    	      ...
    	      $.post('<?php echo U('Admin/News/delAll');?>',{'ids':arr},function(st){
   	    	  	layer.close(index);
    	    	  if(st) {
    	    	  	layer.msg('删除成功!');
    	    	  	location.reload();
